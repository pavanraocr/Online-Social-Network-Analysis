...
def findBfs(g, que, depth, node2distances, node2num_paths, node2parents, maxDepth):
	"""
	Returns the list of edges and nodes with the max depth from r in g
	"""
	...
	while depth <= maxDepth:
		...
		while que:
			...
			neighbors[r] = [x for x in g.neighbors(r)]

		if currentLevel:
			while currentLevel:
				nextNode = currentLevel.popleft()
				for node in neighbors[nextNode]:
					if node2distances[node] > node2distances[nextNode]+1:
						node2parents[node] = set()
						node2parents[node].add(nextNode)
						node2num_paths[node] = node2num_paths[nextNode]
						node2distances[node] = node2distances[nextNode]+1
					elif (node2distances[node] == node2distances[nextNode]+1) and nextNode not in node2parents[node]:
						node2parents[node].add(nextNode)
						node2num_paths[node] += node2num_paths[nextNode]
					que.append(node)
			depth += 1
...
def bfs(graph, root, max_depth):
    ...
    node2distances = defaultdict(lambda: math.inf)
    node2num_paths = defaultdict(float)
    ...
    findBfs(graph, que, 1, node2distances, node2num_paths, node2parents, max_depth)
...
def bottom_up(root, node2distances, node2num_paths, node2parents):
	...
	edgeMap = defaultdict(float)

	bfsLevelMap = {}

	#classifying it into levels 
	for x,y in node2distances.items():
		if y in bfsLevelMap:
			bfsLevelMap[y].append((x,y))
		else:
			bfsLevelMap[y] = [(x,y)]

	#sorting in reverse order
	bfsLevelMap = sorted(bfsLevelMap.items(), key=lambda x: -x[0])

	#from being a dictionary the above function converts it into a list of lists based on level in reverse order
	for levelNum, levelList in bfsLevelMap:
		#it should not be calculated for level 0
		if levelNum == 0:
			continue
		for node,level in levelList:
			if node not in nonLeafNodes:
				#if the node is a leaf node then it is credited with value 1
				nodeWeights[node] = 1
			else:
				#if it is non leaf node then a value 1 is added to the available credit
				nodeWeights[node] += 1

			#checks if there is more than one parent for the node
			if len(node2parents[node]) > 1:
				#then the credit must be devided with the fraction of all the shortest paths
				#(P[i],X) = (node2num_paths(P[i])*nodeWeight[node]) / sumOfAllParentNumPaths
				sumOfAllParentNumPaths = 0
				for parent in node2parents[node]:
					#add all the parents to the non leaf node
					if parent not in nonLeafNodes:
						nonLeafNodes.add(parent)
					sumOfAllParentNumPaths += node2num_paths[parent]
				
				for parent in node2parents[node]:
					#the key for the edge tuple should be sorted (For EX for edge between B and A the key should be ('A','B'))
					key = tuple(sorted((node,parent)))
					edgeMap[key] = (node2num_paths[parent]*nodeWeights[node])/sumOfAllParentNumPaths
					nodeWeights[parent] += edgeMap[key]
			else:
				#then directly append the value of itself to its parent
				#supposed to be the fastest way to retreive the value
				for parent in node2parents[node]:
					break
				if parent not in nonLeafNodes:
					nonLeafNodes.add(parent)
				
				#adding the complete weight to the edge between 2 nodes
				key = tuple(sorted((node,parent)))
				edgeMap[key] = float(nodeWeights[node])
				nodeWeights[parent] += nodeWeights[node]
...
def approximate_betweenness(graph, max_depth):
    ...
    """
    finalEdgeMap = defaultdict(float)

    for n in graph.nodes():
    	node2distances, node2num_paths, node2parents = bfs(graph, n, max_depth)
    	edgeDict = bottom_up(n, node2distances, node2num_paths, node2parents)

    	for edge,value in edgeDict.items():
    		finalEdgeMap[edge] += value

    for edge in finalEdgeMap:
    	finalEdgeMap[edge] = finalEdgeMap[edge]/2
...
def partition_girvan_newman(graph, max_depth):
    ...
    unsortedBetweeness = approximate_betweenness(graphCopy, max_depth)
    approxBetweeness = deque(sorted(unsortedBetweeness.items(), key=lambda x:(-x[1], x[0])))

    components = list(nx.connected_component_subgraphs(graphCopy))
    while len(components) == 1:
    	edge = approxBetweeness.popleft()
    	for u,v in [edge[0]]:
    		break
...
def get_subgraph(graph, min_degree):
    ...
    graphCopy = graph.copy()
    nodes2Keep = set()
    for x in nx.nodes(graphCopy):
    	if len(graphCopy.neighbors(x)) >= min_degree:
    		nodes2Keep.add(x)
    subGraph = graphCopy.subgraph(nodes2Keep)
    return subGraph
...
def volume(nodes, graph):
    ...
    4
    """
    return len(nx.edges(graph,nodes))
...
def cut(S, T, graph):
    ...
    1
    """	
    return len(set([tuple(sorted((x,y))) for x,y in nx.edges(graph,S)]) & set([tuple(sorted((x,y))) for x,y in nx.edges(graph,T)]))
...
def norm_cut(S, T, graph):
    ...
    return 1. * (cutVal/volume(S,graph))+(cutVal/volume(T,graph))
...
def score_max_depths(graph, max_depths):
    ...
    for x in max_depths:
    	...
    	normCutScore = norm_cut(components[0],components[1], graph)
    	finalScores.append((x,normCutScore))
...
def make_training_graph(graph, test_node, n):
    ...
    """
    graphCopy = graph.copy()
    edgesBeingRemoved = sorted(nx.edges(graphCopy,test_node))[:n]
    graphCopy.remove_edges_from(edgesBeingRemoved)
    return graphCopy
...
def jaccard(graph, node, k):
    ...
    sortedList = sorted(scores, key=lambda x: (-x[1],x[0]))
    return sortedList[:k]
...
def path_score(graph, root, k, beta):
    ...
    """
    node2distances, node2num_paths, node2parents = bfs(graph, root, max_depth=5)

    missingLinks = []
    for x,y in nx.non_edges(graph):
    	if root==x:
    		missingLinks.append(y)
    	elif root==y:
    		missingLinks.append(x)

    finalList = defaultdict(float)

    for v in missingLinks:
    	key = (root,v)
    	i = node2distances[v]
    	numOfSP = node2num_paths[v]

    	finalList[key] = (beta**i) * numOfSP

    return sorted(finalList.items(), key=lambda x: (-x[1],x[0]))[:k]
...
def evaluate(predicted_edges, graph):
    ...
    0.5
    """
    listOfEdges = [sorted((x,y)) for x,y in nx.edges(graph)]
    return len([(x,y) for x,y in predicted_edges if sorted((x,y)) in listOfEdges])/len(predicted_edges)
